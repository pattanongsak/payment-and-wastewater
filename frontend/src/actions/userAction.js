...
export const getUserDetails = (id) => async (dispatch) => {
  try {
    ...
    dispatch({ type: USER_DETAIL_SUCCESS, payload: data.usered });
  } catch (error) {
...
export const updateLoginUser = () => async (dispatch) => {
  try {
    dispatch({ type: UPDATE_LOGIN_USER_REQUEST });

    const { data } = await axios.put(`/api/v1/update/login/user/on`);

    dispatch({ type: UPDATE_LOGIN_USER_SUCCESS, payload: data });
  } catch (error) {
    dispatch({
      type: UPDATE_LOGIN_USER_FAIL,
      payload: error.response.data.message,
    });
  }
};

export const updateLoginUserOff = () => async (dispatch) => {
  try {
    dispatch({ type: UPDATE_LOGIN_USER_OFF_REQUEST });

    const { data } = await axios.put(`/api/v1/update/login/user/off`);

    dispatch({ type: UPDATE_LOGIN_USER_OFF_SUCCESS, payload: data });
  } catch (error) {
    dispatch({
      type: UPDATE_LOGIN_USER_OFF_FAIL,
      payload: error.response.data.message,
    });
  }
};
